...
if __name__ == "__main__":
    ...
    for test_session in sessions:
        ...
        # 初始化模型（四分类）
        model = EEGConformer(
            n_chans=channel,  # 与你的输入通道数一致
            n_times=n_timesteps,  # 输入的时间点数
            n_outputs=4,  # 替换为你的分类类别数
            n_filters_time=40,  # 时间滤波器数量（保持论文推荐）
            filter_time_length=25,  # 时间卷积核长度（可调整）
            pool_time_length=16,  # 时间维度池化窗口长度（调整为适配256时间点）
            pool_time_stride=8,  # 池化步长（确保整除）
            att_depth=6,  # 保持原论文Transformer深度
            att_heads=10,  # 注意力头数（保持原论文配置）
            att_drop_prob=0.5,  # 注意力机制dropout
            drop_prob=0.5,  # 卷积层dropout
            final_fc_length="auto",
            input_window_seconds=1,  # 重要！指定输入时间长度（秒）
            sfreq=250,  # 采样率
        ).to(device)

        # 计算参数量和FLOPs（仅第一次迭代时计算）
