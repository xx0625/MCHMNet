...
from model.tcnet_torch import EEGTCNet
...
if __name__ == "__main__":
    ...
    for test_session in sessions:
        ...
        # 初始化模型（四分类）
        model = EEGTCNet( F1=32, D=2, kerSize=32, eeg_chans=61, poolSize=8,
        tcn_filters=64, tcn_kernelSize=4, n_classes=4).to(device)
    ...
    if all_acc:  # 确保有有效数据
        ...
        # 保存结果到Word文档
        model_name = "model6"  # 从导入语句from model6 import EEGNet获取
        params = {
